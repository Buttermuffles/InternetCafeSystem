...
:: 7. Register for auto-start in the Registry (so the tray icon shows up)
echo [5/5] Registering Auto-Start for the GUI...
set "EXE_PATH=%INSTALL_DIR%\%BIN_NAME%"
schtasks /Delete /TN "ICafeMonitorAgent" /F >nul 2>&1
schtasks /Create /TN "ICafeMonitorAgent" /SC ONLOGON /DELAY 000:00:30 /RL HIGHEST /F /TR "\"%EXE_PATH%\"" >nul 2>&1

:: 8. Add firewall exception for Electron
echo [EXTRA] Adding firewall exception...
