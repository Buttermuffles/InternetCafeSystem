...
class VideoController extends Controller
{
    ...
    public function upload(Request $request)
    {
        ...
        if ($contents === false) {
            return response()->json(['status' => 'error', 'message' => 'Invalid base64 video payload.'], 400);
        }
    ...
    public function latest(string $pcName)
    {
        ...
        if (!$record) {
            return response()->json(['status' => 'error', 'message' => 'No video available for this PC.'], 404);
        }

        if (!Storage::exists($record['path'])) {
            return response()->json(['status' => 'error', 'message' => 'Stored video file missing.'], 404);
        }
